component extends="testbox.system.BaseSpec" {
	...
	function run(){
		...
		describe("taffy.core.api", function(){

			it("cacheBeanMetadata resources with conflicting URI values", function(){
				var taffy = new taffy.core.api();
				var factory = {
					getBean: function(beanName){ return new taffy.tests.api.resources.ConflictingURI(); }
				};
				var taffyRef = { status: { skippedResources: [] } };
				var beanList = "foo,bar";

				makePublic(taffy, "cacheBeanMetaData", "public_cacheBeanMetaData");
				var endpoints = taffy.public_cacheBeanMetaData(factory, beanList, taffyRef);

				// debug({endpoints: endpoints, taffyRef: taffyRef});

				var uris = structKeyArray(endpoints);
				expect( endpoints[uris[1]].beanName ).toBe('foo');
				expect( arrayLen(taffyRef.status.skippedResources) ).toBeGT(0);
			});

		});

		describe("NOT YET PORTED", function(){
