...
<a id="download" href="https://github.com/atuttle/Taffy/archive/v3.2.0.zip">Download Taffy 3.2.0.zip</a>
...
#### Expose parsed TaffyRequest data in request scope \([#312](https://github.com/atuttle/Taffy/issues/312)\)

`request._taffyRequest` now includes all information that Taffy has parsed out of the request. This makes it easy to reference in other handlers, like `onError`.

#### Allow renaming and hiding of resources in the dashboard and docs \([#347](https://github.com/atuttle/Taffy/pull/347)\)

See [taffy:dashboard:hide](#taffydashboardhide) and [taffy:docs:hide](#taffydocshide) for hiding, and [taffy:docs:name](#taffydocsname) for renaming. There is no `taffy:dashboard:name` because the name is no longer displayed on the dashboard.

### Breaking Changes

As a minor release (3.1.x to 3.2.0), there are no known breaking changes in this release. Taffy obeys [SemVer 2.0.0](http://semver.org/) versioning rules. If you find that this version breaks something for you, [please let us know by filing an issue](https://github.com/atuttle/Taffy/issues/new)!
