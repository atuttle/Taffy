component hint="Your Application.cfc should extend this class" {
	...
	private void function setupFramework() output="false" {
		...
		local.defaultConfig.returnExceptionsAsJson = false;
	...
	private struct function parseRequest() output="false" {
		...
		if (application._taffy.settings.jsonp != false) {
			if (structKeyExists(requestObj.requestArguments, application._taffy.settings.jsonp)) {
				local.cbName = requestObj.requestArguments[application._taffy.settings.jsonp];
				// validate callback name to prevent XSS via JSONP injection
				if (reFindNoCase("^[a-zA-Z_$][a-zA-Z0-9_$.]*$", local.cbName)) {
					requestObj.jsonpCallback = local.cbName;
				} else {
					throwError(400, "Invalid JSONP callback name");
				}
			}
		}
